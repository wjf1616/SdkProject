...
public class FallingView extends View {
    ...
    private static boolean m_stopAnimate = false;
    ...
    private static  int objNum = 0;//重绘间隔时间
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        int count = fallObjects.size();
        if(fallObjects.size()>0){
//            if(m_stopAnimate && resetNum() == (fallObjects.size())) {
//                // 隔一段时间重绘一次, 动画效果
...
//            }
            for (int i=0;i<count;i++) {
                //然后进行绘制
                fallObjects.get(i).isStopReset(m_stopAnimate);
                if(!fallObjects.get(i).isReset()) {
    ...
    public int getFallObjectNum(){
        if(fallObjects != null)
            return fallObjects.size();
        return  0;
    }

...
public class FallObject {
    ...
    private boolean m_stop;//物体下落过程中风向和风力是否产生随机变化

    private static final int defaultSpeed = 10;//默认下降速度
    private static final int defaultWindLevel = 5;//默认风力等级
    private static final int defaultWindSpeed = 30;//默认单位风速
    ...
    private void moveX(){
        initWindLevel = 1;
        presentX += defaultWindSpeed * Math.sin(angle) + initWindLevel*0.01;
        if(isWindChange){
    ...
    private void reset1(){
        ...
        presentY = random.nextInt((int)(parentHeight-objectHeight))- parentHeight;
//        presentY = -parentHeight -objectHeight;
        randomSpeed();//记得重置时速度也一起重置，这样效果会好很多
        randomWind();//记得重置一下初始角度，不然雪花会越下越少（因为角度累加会让雪花越下越偏）
    }
    /**
    ...
    public boolean isReset(){
        return m_stop && presentY < -objectHeight;
    }
     ...
     * 是否重置位置。
     */
    public void isStopReset(boolean stop){
        m_stop = stop;
    }
